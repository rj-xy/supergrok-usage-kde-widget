...
PlasmoidItem {
    ...
    toolTipMainText: root.entry ? root.entry.label : i18n("Meta AI Usage")
    ...
    function currentCommand() {
        const bundled = Logic.fileUrlToPath(Qt.resolvedUrl("../code/meta-usage-kde-widget"));
        const bin = Logic.resolveBinary(Plasmoid.configuration.binaryPath, bundled, "meta-usage-kde-widget");
        return Logic.buildCommand(bin, root.fetchTimeoutSecs, "meta-usage-kde-widget");
    }
    ...
    Timer {
        ...
        onTriggered: {
            ...
            root.pendingCommand = "";
            root.failure = i18n("Meta AI usage took too long (>%1s)", root.fetchTimeoutSecs);
        }
    }
    ...
    function consume(data) {
        ...
        if (!parsed.ok) {
            const headline = exitCode !== 0
                ? (stderr.trim() ? i18n("Could not fetch Meta AI usage")
                                 : i18n("meta-usage-kde-widget exited with %1", exitCode))
                : i18n("invalid output");
    ...
    function openUsagePage() {
        launcher.exec("xdg-open 'https://www.meta.ai/'");
    }
