...
PlasmoidItem {
    ...
    toolTipMainText: root.entry ? root.entry.label : i18n("SuperGrok Usage")
    ...
    Timer {
        ...
        onTriggered: {
            ...
            root.pendingCommand = "";
            root.failure = i18n("SuperGrok usage took too long (>%1s)", root.fetchTimeoutSecs);
        }
    }
    ...
    function consume(data) {
        ...
        if (!parsed.ok) {
            const headline = exitCode !== 0
                ? (stderr.trim() ? i18n("Could not fetch SuperGrok usage")
                                 : i18n("supergrok-usage-kde-widget exited with %1", exitCode))
                : i18n("invalid output");
    ...
    function openUsagePage() {
        launcher.exec("xdg-open 'https://grok.com/?_s=usage'");
    }
